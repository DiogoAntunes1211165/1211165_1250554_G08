...
public class AuthorServiceImpl implements AuthorService {
    ...
    private Author create(String authorNumber, String name, String bio, String photo) {
        if (authorRepository.findByAuthorNumber(authorNumber).isPresent()) {
            throw new ConflictException("Author with author number " + authorNumber + " already exists");
        }

        Author author = new Author(authorNumber, name, bio, photo);
        return authorRepository.save(author);
    }
    ...
    public Author update(AuthorViewAMQP authorViewAMQP) {
        ...
        author.applyPatch(Long.parseLong(authorViewAMQP.getVersion()), authorViewAMQP.getName(), authorViewAMQP.getBio(), authorViewAMQP.getPhoto());

        Author updatedAuthor = authorRepository.save(author);

        if (updatedAuthor != null) {
            authorEventsPublisher.sendAuthorUpdated(updatedAuthor, updatedAuthor.getVersion().toString());
        }
